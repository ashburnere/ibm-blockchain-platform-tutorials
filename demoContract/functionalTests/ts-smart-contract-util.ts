...
export class SmartContractUtil {
    ...
    public static async getConnectionProfile() {
        const connectionProfilePath: string = '/home/erik/.fabric-vscode/gateways/local_fabric/local_fabric.json';
    ...
    public static async submitTransaction(contractName: string, functionName: string, args: string[], gateway): Promise<Buffer> {
        ...
        if (contractName !== '') {
            contract = await network.getContract('demoContract', contractName);
        } else {
            contract = await network.getContract('demoContract');
        }
